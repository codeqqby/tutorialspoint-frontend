...
如需了解更多有关 autocomplete 部件的细节，请查看 API 文档 [自动完成部件（Autocomplete Widget）](api-autocomplete.html)。
...
这是一个不被支持的不完美的部件。这里纯粹是为了演示 autocomplete 定制功能。[如需了解更多有关该部件工作原理的细节，请点击这里查看相关的 jQuery 文章。](//www.learningjquery.com/2010/06/a-jquery-ui-combobox-under-the-hood/)
...
在本实例中，数据源是 [geonames.org webservice](//geonames.org/)。虽然选择一个元素后文本域中是该城市名称，但是会显示更多的信息以便找到正确的条目。数据也可以回调，显示在下面的结果框中。
